...
echo "============================================================"
echo " STOREMAN WEB AUTH-FIRST MASTER FIX"
echo " Sign In + Sign Up + Supabase + Pending + Admin + Multi-device"
echo "============================================================"
...
mkdir -p "backup/web_auth_first_$STAMP"

cp index.html \
   "backup/web_auth_first_$STAMP/index.html.before"

if [ -f storeman-web-auth-gate.js ]; then
    cp storeman-web-auth-gate.js \
       "backup/web_auth_first_$STAMP/storeman-web-auth-gate.js.before"
fi

echo "Backup created."

# ------------------------------------------------------------
# 2. WRITE NEW AUTH-FIRST GATE
# ------------------------------------------------------------

cat > storeman-web-auth-gate.js <<'JS'
(function () {
    "use strict";
...
echo "New auth gate written."

# ------------------------------------------------------------
# 3. CACHE-BUST THE AUTH SCRIPT
# ------------------------------------------------------------
...
old = '<script src="storeman-web-auth-gate.js"></script>'

new = '<script src="storeman-web-auth-gate.js?v=20260827authfinal"></script>'
...
if old in s:
    ...
    import re
    s = re.sub(
        r'<script[^>]+storeman-web-auth-gate\.js[^>]*></script>',
        new,
        s
    )
...
test -f storeman-web-auth-gate.js
echo "Auth gate: OK"

grep -q "storeman-login-email" storeman-web-auth-gate.js
echo "Sign In email field: OK"

grep -q "storeman-signup-email" storeman-web-auth-gate.js
echo "Sign Up email field: OK"

grep -q "getSession" storeman-web-auth-gate.js
echo "Supabase session: OK"

grep -q "profiles" storeman-web-auth-gate.js
echo "Profile check: OK"

grep -q "Account Pending" storeman-web-auth-gate.js
echo "Pending flow: OK"

grep -q "20260827authfinal" index.html
echo "Cache bust: OK"
...
if command -v node >/dev/null 2>&1; then
    node --check storeman-web-auth-gate.js
    echo "JavaScript syntax: OK"
else
    echo "Node not installed; skipped node syntax check."
fi
...
git add index.html storeman-web-auth-gate.js \
    "backup/web_auth_first_$STAMP"

git commit -m "Fix web auth first login timeout and multi-device access"
...
echo
echo "============================================================"
echo " STOREMAN WEB AUTH-FIRST FIX DEPLOYED"
echo "============================================================"
echo
echo "IMPORTANT:"
echo "1. Open the link in another phone."
echo "2. Wait 2-5 seconds."
echo "3. Sign In screen must appear immediately."
echo "4. Sign Up must be available."
echo "5. Dashboard must NOT appear before authentication."
echo "6. Pending users must see Account Pending."
echo "7. Active users can enter Dashboard."
echo "8. Normal users cannot see Settings/User Management."
echo
echo "LIVE LINK:"
echo "https://villagevictor.github.io/Storeman-app/"
echo
echo "If browser cache remains, open the link in Incognito."
echo
