...
BACKUP="backup/auth_first_$TIME"

mkdir -p "$BACKUP"

cp index.html "$BACKUP/index.html.before-auth-first"

[ -f storeman-auth-final.js ] && \
cp storeman-auth-final.js "$BACKUP/storeman-auth-final.js.before-auth-first" || true

[ -f storeman-pending-ui.js ] && \
cp storeman-pending-ui.js "$BACKUP/storeman-pending-ui.js.before-auth-first" || true

echo "Backup created:"
echo "$BACKUP"

# ============================================================
# 3. CREATE WEB AUTH-FIRST GATE
# ============================================================
...
marker = 'storeman-web-auth-gate.js'

if marker not in s:

    tag = '\n<script src="storeman-web-auth-gate.js"></script>\n'
    ...
    print("Auth gate connected to index.html")
else:
    print("Auth gate already connected")
PY
...
echo
echo "===== SUPABASE CLIENT CHECK ====="

if grep -Eqi "supabase|createClient|@supabase" index.html; then
    echo "Supabase reference detected."
else
    echo "WARNING: Supabase reference was NOT detected in index.html."
fi
...
grep -q "storeman-auth-gate" storeman-web-auth-gate.js
echo "Auth gate: OK"

grep -q "signInWithPassword" storeman-web-auth-gate.js
echo "Sign In: OK"

grep -q "signUp" storeman-web-auth-gate.js
echo "Sign Up: OK"

grep -q "getSession" storeman-web-auth-gate.js
echo "Session check: OK"

grep -q "status.*active" storeman-web-auth-gate.js
echo "Active profile check: OK"
...
git add -A backup/auth_first_$TIME 2>/dev/null || true

git commit -m "Fix web auth-first sign in sign up multi-device access" || true
...
echo
echo "============================================================"
echo " STOREMAN AUTH-FIRST FIX COMPLETE"
echo "============================================================"
...
echo "EXPECTED FLOW:"
echo "1. Open link"
echo "2. Sign In / Sign Up appears FIRST"
echo "3. No Dashboard before authentication"
echo "4. Supabase Auth checks the user"
echo "5. Profile status is checked"
echo "6. Pending user sees Pending screen"
echo "7. Admin approves user"
echo "8. Active user enters Dashboard"
echo "9. Existing application features remain underneath"
echo
echo "IMPORTANT:"
echo "GitHub Pages may take a short time to publish the new commit."
echo
echo "============================================================"
